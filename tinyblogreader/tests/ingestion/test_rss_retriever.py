...
def test_fetch_feed_handles_missing_fields(monkeypatch):
    monkeypatch.setattr(
        "tinyblogreader.ingestion.rss_retriever.feedparser.parse",
        _fake_parse([{}]),
    )
...
def test_fetch_feed_returns_empty_list_for_no_entries(monkeypatch):
    monkeypatch.setattr(
        "tinyblogreader.ingestion.rss_retriever.feedparser.parse",
        _fake_parse([]),
    )
...
def test_fetch_feed_returns_empty_list_on_parse_error(monkeypatch):
    def raise_error(feed_url):
